 					...
 					<table class="table table-striped table-hover">
							<thead>
								<tr >
									...
									<th >tag</th>
									<th >제목</th>
									<th class="cell100 column4">글쓴이</th>
									<th class="cell100 column5">시간</th>
								</tr>
							</thead>
						<c:forEach items="${mo_list }" var="m" >
<tr class="row100 body" >
<td class="cell100 column1">${m.no }</td><td class="cell100 column2">${m.tag }</td><td class="cell100 column3"><a href="mojip?command=detail&title=${m.title }" >${m.title }</a></td><td class="cell100 column4">${m.id }</td><td class="cell100 column5">${m.time }</td>
</tr>
</c:forEach>

<c:if test="${tag ne 'none' }"><a href="mojip?command=my">글쓰기</a></c:if>
</table>
